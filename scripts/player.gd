...
func move_player(player_direction : Vector2, delta : float):
    ...
    if Input.is_action_just_pressed("ui_up"):
        apply_central_force(Vector2(JUMP_VELOCITY,0))
        linear_velocity.y = JUMP_VELOCITY
    
    if (player_direction.x != 0):
        linear_velocity.x = (roundf(player_direction.x) * player_speed) * delta * 1000
    else:
        linear_velocity.x = move_toward(linear_velocity.x, 0, player_speed)
    
    if Input.is_action_pressed("sprint"):
        linear_velocity.x = (roundf(player_direction.x) * player_sprint_speed) * delta * 1000
